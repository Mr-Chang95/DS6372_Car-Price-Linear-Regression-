...
#training the model via 10fold cv
knn3.fit2<-train(log(selling_price)~year + log(km_driven) + fuel+ seller_type + transmission + owner + mileage + log(engine) + log(max_power) + log(seats) + brand,
                  data=car_details,
                  method="knn",
                  trControl=fitControl,
...
#training the model via 10fold cv
knn4.fit2<-train(log(selling_price)~seller_type + owner + log(max_power)+log(seats)+year:log(km_driven)+year:seller_type+year:owner+year:log(max_power)+year:log(seats)+log(km_driven):log(engine)+log(km_driven):log(max_power)+log(km_driven):log(seats)+fuel:seller_type,
                  data=car_details,
                  method="knn",
                  trControl=fitControl,
               tuneGrid=expand.grid(k=c(5:9))
